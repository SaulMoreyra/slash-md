import type { HostToWebview } from "../../src/domain/protocol";
...
export function createEditorContext(): EditorContext {
  ...
  const bar = mountBar(vscode, boot.init, () => flushMarkdown(ctx));
  const frontmatter = mountFrontmatter(vscode, boot.frontmatter);
  const icon = mountIcon({ vscode }, boot.frontmatter);
  ...
  ctx.handles.bar = bar;
  ctx.handles.chrome = mountThreadChrome(vscode);
  ctx.handles.reviewContext = mountReviewContext(vscode);
...
export function startEditorApp(): void {
