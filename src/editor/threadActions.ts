...
import { docsContentRemotePath, relativePosix } from "../workspace/docsWorkspace";
import { getDraftMeta, patchDraftMeta } from "../sidecar/draftMeta";
import { getPull } from "../github/api";
...
import { HostToWebview } from "../domain/messaging";
import { reviewThreadTarget } from "./threadGate";
...
export async function handleThreadReply(
  ...
  const { config, session, prNumber, remotePath } = gate;
...
export async function handleThreadCreate(
  ...
  const { config, meta, session, prNumber, remotePath } = gate;

  const sidecar = document.uri.path.endsWith(".slash.md");
  const localHash = markdownHash(markdown);
  const ahead = Boolean(meta.remoteHash && localHash !== meta.remoteHash);
  if (sidecar && (ahead || !meta.remoteOid)) {
  ...
  let commitId = fresh.remoteOid;
  const path = fresh.remotePath ?? remotePath;
  const commentPr = fresh.prNumber ?? prNumber;
  if (!commitId && commentPr) {
    try {
      const pr = await getPull(session.accessToken, config, commentPr);
      commitId = pr.head.sha;
    } catch {
      commitId = undefined;
    }
  }
  if (!commitId || !path || !commentPr) {
...
export async function pushThreads(
  ...
  const target = reviewThreadTarget({
    ...
    draftPr: meta.prNumber,
    fileRemotePath: await wikiFileRemotePath(document.uri, config),
    draftRemotePath: meta.remotePath,
  });
  if (!target) {
    await webview.postMessage({ type: "threads", threads: [], canWrite: false } satisfies HostToWebview);
    return;
...
> {
  ...
  return { config, meta, session, prNumber: target.prNumber, remotePath: target.remotePath };
}

async function wikiFileRemotePath(
  uri: vscode.Uri,
  config: NonNullable<ReturnType<typeof getContentConfig>>,
  ...
  return (await docsContentRemotePath(uri, config)) ?? relativeFromWorkspace(uri);
}

function relativeFromWorkspace(uri: vscode.Uri): string | undefined {
