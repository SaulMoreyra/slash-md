...
export async function handlePublish(
  ...
  if (!document.uri.path.endsWith(".slash.md")) {
    await handleWikiPublish(context, document, webview, markdown, config);
    return;
  }
...
async function handleWikiPublish(
  ...
  const prNumber = parsePrNumber(splitFrontmatter(markdown).fields.pr);
  if (!prNumber) {
    await vscode.window.showWarningMessage("Publish is disabled until there is an open PR.", {
      modal: true,
    });
    const status = draftBarState(getDraftMeta(context, document.uri), markdown, { mode: "workspace" });
    await webview.postMessage({
      type: "status",
      kind: status.kind,
  ...
  const result = await publishBatch(context, { preferredPr: prNumber });
  if (!result) {
    const status = draftBarState(getDraftMeta(context, document.uri), document.getText(), {
      mode: "workspace",
    });
    await webview.postMessage({
      type: "status",
  ...
  let onDisk = markdown;
  try {
    onDisk = Buffer.from(await vscode.workspace.fs.readFile(document.uri)).toString("utf8");
  } catch {
    onDisk = document.getText();
  }
  await webview.postMessage({ type: "frontmatter", fields: splitFrontmatter(onDisk).fields } satisfies HostToWebview);
  const publishedUrl = `https://github.com/${config.owner}/${config.name}/blob/${config.defaultBranch}/${result.paths[0] ?? ""}`;
  await webview.postMessage({
    type: "status",
    kind: "published",
    label: "published",
    publishEnabled: false,
    prUrl: result.paths[0] ? publishedUrl : result.pr.html_url,
    path: result.paths[0],
    repoMode: "workspace",
  } satisfies HostToWebview);
}
