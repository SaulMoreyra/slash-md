...
export function getConfiguredSections(contentPath: string): string[] {
  ...
  for (const section of raw) {
    ...
    if (trimmed === contentPath || trimmed.startsWith(`${contentPath}/`)) {
      out.add(trimmed);
    } else {
      out.add(posixJoin(contentPath, trimmed));
    }
  }
  return [...out].sort();
}
...
function normalizeFile(raw: Record<string, unknown> | SlashmdFile): SlashmdFile {
  ...
  if (typeof raw.contentPath === "string" && raw.contentPath.trim()) {
    out.contentPath = raw.contentPath.trim().replace(/^\/+|\/+$/g, "");
  }
