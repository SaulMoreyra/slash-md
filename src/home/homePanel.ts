...
export class HomePanel {
  ...
  private async handleRenameFolder(folderPath: string): Promise<void> {
    ...
    const { posixBasename, posixDirname, posixJoin } = await import("../domain/paths");
    const { slugify } = await import("../domain/slug");
  ...
  private async handleOpenIndex(): Promise<void> {
    ...
    const { posixJoin } = await import("../domain/paths");
