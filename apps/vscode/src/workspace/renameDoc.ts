...
import { clearDraftMeta, findDraftByRemotePath } from "../sidecar/draftLookup";
...
import { resolveContentConfig, upsertDraft } from "../sidecar/openFromGithub";
import { pickMoveTarget } from "./sections";
import { draftBarState } from "../editor/barState";
import { notifyEditor } from "../editor/editorLive";
import { rewriteLinksForMove } from "@slash-md/core/links";
...
async function deleteDraftUri(
  ...
  const meta = getDraftMeta(context, target);
  const text = Buffer.from(await vscode.workspace.fs.readFile(target)).toString("utf8");
  const title = labeledTitle(text, target.path.split("/").pop() ?? "doc");
