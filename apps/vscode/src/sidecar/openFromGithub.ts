...
export async function openRemotePath(
  ...
  if (root) {
    const localUri = vscode.Uri.joinPath(root, ...remotePath.split("/").filter(Boolean));
    try {
      await vscode.workspace.fs.stat(localUri);
      await vscode.commands.executeCommand("vscode.openWith", localUri, SlashMdEditorProvider.viewType);
      return;
    } catch {
      // not on disk — fall through to sidecar
    }
  }
